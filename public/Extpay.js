var ExtPay = (function () {
  ...
  function ExtPay(extension_id) {
    ...
    browserPolyfill.management &&
      browserPolyfill.management.getSelf().then(async (ext_info) => {
...
`;
    ...
    async function fetch_user() {
      ...
      if (!api_key) {
        return {
          ...
          installedAt: storage.extensionpay_installed_at
            ...
            : new Date(), // sometimes this function gets called before the initial install time can be flushed to storage
          trialStartedAt: null,
        };
      }
      ...
      if (parsed_user.paidAt) {
        if (
          !storage.extensionpay_user ||
          (storage.extensionpay_user && !storage.extensionpay_user.paidAt)
        ) {
    ...
    return {
      ...
      startBackground: function () {
        browserPolyfill.runtime.onMessage.addListener(function (
          message,
          sender,
          send_response
        ) {
